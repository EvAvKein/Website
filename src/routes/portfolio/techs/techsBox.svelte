<section>
  {#each techsByNameAndSrc as tech}
    <button
      on:click={() => {selectedTech = tech[0]}}
      on:blur={() => {setTimeout(() => {if (selectedTech === tech[0]) selectedTech = null}, 100)}}
    >
      <img src={tech[1]} alt={tech[0] + "'s logo"}/>
    </button>
  {/each}
</section>
...
<style>
  ...
  button:hover img, button:focus img {
    ...
    filter: drop-shadow(0 0 0.35em var(--highlightColor));
  }
</style>